  ...
  return (
    <div className='font-serif grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 p1 md:p-3 '>
      {data?.map(({value,icon_url,id})=>{
        return<div className=' text-white md:m-3 group md:hover:shadow-md hover:border border-sky-500 bg-[#1e293b] rounded-md group m-1 p-1 md:p-3  sm:hover:scale-105 transition-all duration-200 ease-in transform cursor-pointer' key={id}>
        <p >{value}</p>
