      ...
      {quotes?.length == 0 ? (
        ...
        quotes?.map((quote) => {
          return (
            <Fade bottom>
            <div
              className="p-2 rounded hover:border border-sky-400 inline text-white bg-[#1e293b] group my-1 md:m-3  md:p-3  transition-all duration-200 ease-in transform cursor-pointer  "
              key={quote?.quote}
            >
